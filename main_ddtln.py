...
def parse_args():
    ...
    parser.add_argument('--cwru_data', type=str, default="H:\\DDTLN\\12k_de_cwru_data_load2_0.007.npy", help='')
    parser.add_argument('--cwru_label', type=str, default="H:\\DDTLN\\12k_de_cwru_label_load2_0.007.npy", help='')
    parser.add_argument('--bjtu_data', type=str, default="H:\\DDTLN\\gt300data_data.npy", help='')
    parser.add_argument('--bjtu_label', type=str, default="H:\\DDTLN\\gt300data_label.npy", help='')
...
def load_data():
    ...
    target_label = np.load(args.bjtu_label)
...
def _mix_rbf_mmd2(m, n, X, Y, sigmas=(10,), wts=None, biased=True):
...
def MDA(m, n, source, target, bandwidths=[1]):
    ...
    cs = (torch.matmul(torch.t(source), source) - torch.matmul(torch.t(tmp_s), tmp_s) / ns) / (ns - 1 + eps) * (
                ns / m)

    # target covariance
    tmp_t = torch.matmul(torch.ones(size=(1, nt)).cuda(), target)
    ct = (torch.matmul(torch.t(target), target) - torch.matmul(torch.t(tmp_t), tmp_t) / nt) / (nt - 1 + eps) * (
                nt / n)
    # frobenius norm
...
def _classification_division(data, label):
    label = torch.argmax(label, dim=-1)
    N = data.size()[0]
    a, b, c, d = [], [], [], []
    for i in range(N):
        if label[i] == 0:
            a.append(data[i].detach().cpu().numpy())
        elif label[i] == 1:
            b.append(data[i].detach().cpu().numpy())
        elif label[i] == 2:
            c.append(data[i].detach().cpu().numpy())
        elif label[i] == 3:
            d.append(data[i].detach().cpu().numpy())
    return torch.from_numpy(np.array(a)).cuda(), torch.from_numpy(np.array(b)).cuda(), torch.from_numpy(np.array(c)).cuda(), torch.from_numpy(np.array(d)).cuda()


def CDA(m, n, output1, source_label, output2, pseudo_label):
    s_0, s_1, s_2, s_3 = _classification_division(output1, source_label)
    t_0, t_1, t_2, t_3 = _classification_division(output2, pseudo_label)
...
class I_Softmax(nn.Module):
    ...
    def __init__(self, m, n, source_output1, source_label):
        ...
        self.label_argmax = torch.argmax(self.source_label, dim=-1)
    ...
    def _combine(self):
        ...
        if len(a) != 0:
            ...
            self.la = np.array([item.cpu().detach().numpy() for item in self.la])
            self.label_set.append(self.la)
        if len(b) != 0:
            self.data_set.append(b)
            self.lb = np.array([item.cpu().detach().numpy() for item in self.lb])
            self.label_set.append(self.lb)
        if len(c) != 0:
            self.data_set.append(c)
            self.lc = np.array([item.cpu().detach().numpy() for item in self.lc])
            self.label_set.append(self.lc)
        if len(d) != 0:
            self.data_set.append(d)
            self.ld = np.array([item.cpu().detach().numpy() for item in self.ld])
            self.label_set.append(self.ld)
        data = torch.vstack(self.data_set)
        label = np.vstack(self.label_set)
        return data.cuda(), torch.from_numpy(label).cuda()
    ...
    def _class_angle(self, a, la):
        ...
        for i in range(len(a)):
            ...
            part2 = c[index + 1:]
            if c[index] > 0:
                val = [c[index] / self.m - self.n]
            elif c[index] <= 0:
                val = [c[index] * self.m - self.n]
            if i == 0:
                new_tensor = torch.concat((part1, torch.tensor(val, device='cuda:0'), part2), dim=0)
            else:
                tensor = torch.concat((part1, torch.tensor(val, device='cuda:0'), part2), dim=0)
    ...
    def forward(self):
        ...
        return F.softmax(data, dim=-1), label, loss
...
def train(model, epoch, source_loader, target_loader, optimizer):
    ...
    for i in range(0, num_iter):
        ...
        optimizer.zero_grad()
        output1 = model(source_data)                    #, output2, target_data
        output2 = model(target_data)
        data, label, clc_loss_step = I_Softmax(1, 0, output1, source_label).forward()
        pre_pseudo_label = F.one_hot(torch.argmax(F.softmax(output2, dim=-1), dim=-1), num_classes=args.num_classes)
        ...
        optimizer.step()
        metric_accuracy_1.update(F.softmax(data, dim=-1).max(1)[1], label.max(1)[1])
        metric_accuracy_2.update(F.softmax(output2, dim=-1).max(1)[1], target_label.max(1)[1])
        metric_mean_1.update(loss_step)
        metric_mean_2.update(criterion(output1, source_label.float()))
...
if __name__ == '__main__':
    ...
    for epoch in range(0, args.nepoch):
        ...
        print('Epoch{}, train_loss is {:.5f},test_loss is {:.5f}, train_accuracy is {:.5f},test_accuracy is {:.5f},train_all_loss is {:.5f},target_cla_loss is {:.5f},source_cla_loss is {:.5f},cda_loss is {:.5f},mda_loss is {:.5f}'.format(
                    epoch + 1,train_loss,test_loss,train_acc,test_acc,train_all_loss,target_cla_loss,source_cla_loss,cda_loss,mda_loss))
