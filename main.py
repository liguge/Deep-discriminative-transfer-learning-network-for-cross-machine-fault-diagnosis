...
def MDA(m, n, source, target, bandwidths=[1]):
    ...
    kernel_loss = _mix_rbf_mmd2(m, n, source, target, sigmas=bandwidths) * 10
    eps = 1e-5
    d = source.size()[1]
...
def train(model, epoch, source_loader, target_loader, optimizer):
    ...
    for i in range(0, num_iter):
        ...
        pseudo_data, pseudo_label, pseudo_loss_step = I_Softmax(1, 0, output2, pre_pseudo_label).forward()
        CDA_loss = CDA(m, n, output1, source_label, output2, pre_pseudo_label)
        MDA_loss = MDA(m, n, output1, output2)
        # loss_step = criterion(output1, source_label)
        ...
        optimizer.step()
        metric_accuracy_1.update(F.softmax(data, dim=-1).max(1)[1], label)
        metric_accuracy_2.update(F.softmax(output2, dim=-1).max(1)[1], target_label)
