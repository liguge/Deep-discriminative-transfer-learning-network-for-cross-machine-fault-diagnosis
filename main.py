...
def parse_args():
    ...
    parser.add_argument('--cwru_data', type=str, default="H:\\DDTLN\\48kcwru_data.npy", help='')
    parser.add_argument('--cwru_label', type=str, default="H:\\DDTLN\\48kcwru_label.npy", help='')
    ...
    parser.add_argument('--losses', type=list, default=[], help='')
...
def _mmd2(m, n, K_XX, K_XY, K_YY, const_diagonal=False, biased=False):
    if biased:
        mmd2 = ((torch.sum(K_XX) / (m * m + 1e-5))
                + (torch.sum(K_YY) / (n * n + 1e-5))
                - (2 * torch.sum(K_XY) / (m * n + 1e-5))).float()
    else:
...
def MDA(m, n, source, target, bandwidths=[1]):
    ...
    ct = (torch.matmul(torch.t(target), target) - torch.matmul(torch.t(tmp_t), tmp_t) / (nt + eps)) / (nt - 1 + eps) * (
                nt / (n + eps))
    # frobenius norm
...
def _classification_division(data, label):
    ...
    return _if_list(a), _if_list(b), _if_list(c), _if_list(d)
...
class DDTLN(nn.Module):
    def __init__(self):
        ...
        self.p7_1 = nn.Sequential(nn.Linear(512, 128))
        self.p7_2 = nn.Sequential(nn.utils.weight_norm(nn.Linear(128, 4), name='weight'))
        # self._weights_init()
    def forward(self, x, y):
        ...
        y1 = self.p7_1(self.p6(y).squeeze())
        y2 = self.p7_2(y1)
        return x1, x2, y1, y2
...
def train(model, epoch, source_loader, target_loader, optimizer):
    ...
    for i in range(0, num_iter):
        ...
        optimizer.zero_grad()
        out1, output1, out2, output2 = model(source_data, target_data)                    #, output2, target_data
        data, label, clc_loss_step = I_Softmax(3, 16, output1, source_label).forward()
        pre_pseudo_label = torch.argmax(F.softmax(output2, dim=-1), dim=-1)
        pseudo_data, pseudo_label, pseudo_loss_step = I_Softmax(3, 16, output2, pre_pseudo_label).forward()
        # = criterion(output1, source_label)
        CDA_loss = CDA(m, n, out1, source_label, out2, pre_pseudo_label)
        MDA_loss = MDA(m, n, out1, out2)
        loss_step = clc_loss_step + (MDA_loss + 0.5 * CDA_loss) + 10 * pseudo_loss_step
        loss_step.backward()
        optimizer.step()
